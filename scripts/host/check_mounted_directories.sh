...
if [[ ! -f "${vagrant_dir}/etc/guest/mysql/my.cnf" ]]; then
    error "Directory '${vagrant_dir}/etc' was not mounted as expected by Vagrant.
        Please make sure that 'p19x30/magento2.ubuntu' Vagrant box was downloaded successfully (if not, this may help http://stackoverflow.com/questions/35519389/vagrant-cannot-find-box)
