...
if [[ ${host_os} == "Windows" ]] || [[ ${use_nfs} == 0 ]]; then
    ...
    To verify that deployment configuration for $(bold)${magento_ce_dir}$(regular) in PhpStorm is correct,
        use instructions provided here: $(bold)https://github.com/p19x30/magento2-vagrant-for-developers/blob/2.0/docs/phpstorm-configuration-windows-hosts.md$(regular).
